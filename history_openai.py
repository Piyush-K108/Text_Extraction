...
def json_to_file(json_data, file_id, thread_id, run_id, output_file='Json_output.json'):
    try:
...
def process_code_files(file_paths):
    try:
        ...
        text = "Just follow the instructions given and extract all data from the file completely, without omitting any rows or fields. Ensure that the structure is preserved and unique values are maintained, without introducing duplicate entries or skipping rows."
        ...
        if len(messages) > 0 and len(messages[0].content) > 0:
            ...
            try:
                ...
                try:
                    ...
                    cleaned_json_string = cleaned_json_string.strip("```").strip()
                    json_data = eval(cleaned_json_string)
            except Exception as e:
                print("Error", e)
